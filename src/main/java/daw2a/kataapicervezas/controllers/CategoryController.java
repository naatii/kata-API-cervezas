...
public class CategoryController {
    ...
    @PostMapping
    public ResponseEntity<Category> agregarCategoria(@RequestBody @Valid Category category) {
        Category nuevaCategory = categoryService.agregarCategoria(category);
        return ResponseEntity.status(HttpStatus.CREATED).body(nuevaCategory);
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Category> borrarCategoria(@PathVariable Long id) {
        try {
            categoryService.borrarCategoria(id);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Category> actualizarCategoria(@PathVariable Long id,
                                                        @RequestBody @Valid Category categoriaActualizada) {
        try {
            Category category = categoryService.actualizarCategoria(id, categoriaActualizada);
            return ResponseEntity.ok(category);
        } catch (NoSuchElementException ex) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
        }
